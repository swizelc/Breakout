...
def play():
    ...
    while playing:
        ...
        for i in balls:
            ...
            if death:
                ...
                if lives ==0:
                    ...
                    #get a new ball
                    balls.remove(i)
                    total_sprites.remove(i)
                    ball3 = Ball()
                    total_sprites.add(ball3)
                    balls.add(ball3)

        #if paddle and ball collide
        #make it bounce and change its direction
        #by the differece in the centres of the 2 objects
        hit_ball = pygame.sprite.spritecollide(paddle,balls,False)
        for i in hit_ball:
            change = (paddle.rect.x + paddle.w/2) - (i.rect.x+i.w/2)
            i.rect.y = 349
            i.bounce(change)
        ...
        for j in balls:
            ball_brick = pygame.sprite.spritecollide(j,bricks,True)
            if len(ball_brick)>0:
                for i in ball_brick:
                    points += 10
                j.bounce(0)
